...
> Image representation as a superposition of parametric primitives offers a compelling alternative to conventional pixel-based formats, enabling resolution-independent synthesis and efficient real-time playback. While 3D Gaussian Splatting has recently demonstrated the power of differentiable rendering for novel view synthesis, its underlying optimization and alpha-compositing pipeline can be repurposed for 2D image encoding without any camera geometry. In this work, we formulate the task of fitting a single raster image as the minimization of a reconstruction loss between the target and a render composed of multiple anisotropic 2D Gaussian kernels, each carrying spatial, color, and opacity attributes. We leverage tile-based culling, stochastic sampling, and a combined L1-SSIM loss to achieve high fidelity, and we employ Adam optimization with adaptive learning rate decay and periodic density control (splitting and pruning) to obtain a compact set of primitives. The final parameters are exported either as constant arrays embedded in GLSL shaders or as texture-based lookup tables, allowing real-time image reconstruction on commodity GPUs. This work employs a set of two-dimensional anisotropic Gaussian functions—each defined by position, axis lengths (scales), orientation, color, and opacity—and blends them at the pixel level via alpha compositing (i.e., ordered front-to-back blending) to approximate a single static raster image, and mathematically this falls into the categories of function approximation and image decomposition where the target image is represented as a superposition of spatially localized basis functions, with the parameters optimized through differentiable rendering and gradient-based minimization of a reconstruction loss, and notably this operates entirely in screen space with no camera model, depth, or multi-view data, distinguishing it from 3D Gaussian Splatting, and it is essentially an image encoding technique that yields a compact set of pre-computed primitives for real-time shader-based synthesis.
